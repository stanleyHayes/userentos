...
export function confirmAiSharing(path: string): Promise<boolean> {
  const message = aiSharingMessage(path)
  if (Platform.OS === 'web') return Promise.resolve(globalThis.confirm(message))
  return new Promise(resolve => Alert.alert('Share this request with AI providers?', message, [
    { text: 'Cancel', style: 'cancel', onPress: () => resolve(false) },
    { text: 'Allow and send', onPress: () => resolve(true) },

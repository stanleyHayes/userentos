...
export function Logo({ size = 32, variant = 'full', theme = 'dark', className = '' }: LogoProps) {
  const s = size
  const reactId = useId()
  const id = `logo-${theme}-${reactId.replace(/:/g, '')}`
  ...
  const mark = (
    <svg width={s} height={s} viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg">
      <defs>
        ...
        <linearGradient id={`${id}-gold`} x1="0" y1="0" x2="64" y2="64">
          <stop stopColor="#fbbf24" />
          <stop offset="1" stopColor="#f59e0b" />
        </linearGradient>
        {/* Green accent */}
        <linearGradient id={`${id}-green`} x1="20" y1="40" x2="44" y2="56">
      ...
      <rect width="64" height="64" rx="16" fill={theme === 'dark' ? `url(#${id}-bg)` : 'rgba(255,255,255,0.12)'} />
      ...
      {/* Main roof — thick golden chevron with shadow */}
      <path d="M8 30L32 8L56 30" stroke="black" strokeWidth="4.5" strokeLinecap="round" strokeLinejoin="round" fill="none" opacity="0.15" />
      <path d="M8 30L32 8L56 30" stroke={`url(#${id}-gold)`} strokeWidth="4" strokeLinecap="round" strokeLinejoin="round" fill="none" filter={`url(#${id}-glow)`} />

      {/* Secondary inner roof line */}
      <path d="M16 28L32 14L48 28" stroke="white" strokeWidth="1" strokeLinecap="round" strokeLinejoin="round" fill="none" opacity="0.2" />

      {/* Building body */}
      <rect x="14" y="30" width="36" height="24" rx="2" fill="white" fillOpacity={theme === 'dark' ? '0.92' : '0.85'} />

      {/* Building facade details - horizontal lines */}
      ...
      {/* Chimney with smoke */}
      <rect x="42" y="12" width="6" height="18" rx="1.5" fill={theme === 'dark' ? '#1a2d47' : '#e2e8f0'} />
      <rect x="41" y="11" width="8" height="3" rx="1" fill={theme === 'dark' ? '#1a2d47' : '#e2e8f0'} />
      ...
      {/* Connection lines from building — "OS" / tech vibe */}
      ...
      {/* Foundation / base with green accent */}
  ...
  return (
    <span className={`inline-flex items-center gap-2.5 ${className}`}>
      ...
      <span className="flex flex-col leading-none">
        <span className="font-extrabold font-display tracking-tight" style={{ color: theme === 'dark' ? '#1e3a5f' : '#ffffff', fontSize: s * 0.5 }}>
          Rent<span style={{ color: '#f59e0b' }}>OS</span>
        </span>
        <span className="font-semibold tracking-[0.15em] uppercase" style={{ color: theme === 'dark' ? '#94a3b8' : 'rgba(255,255,255,0.45)', fontSize: s * 0.22, marginTop: 1 }}>
          Ghana
        </span>
      </span>
    </span>

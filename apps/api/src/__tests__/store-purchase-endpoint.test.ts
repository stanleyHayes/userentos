...
describe('authenticated purchase and restore completion API', () => {
  ...
  it('refuses a license-test purchase for an account that is not allowlisted', async () => {
    mocks.complete.mockRejectedValue(new StoreVerificationError('test_purchase'))
    const response = await send()
    expect(response.status).toBe(403)
    expect((await response.json()).error).toContain('test purchases are not accepted for this account')
  })

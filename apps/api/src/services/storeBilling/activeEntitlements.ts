...
export async function activateGoogleEntitlements(userId: string, purchaseId: string, revision: number, now = new Date()) {
  ...
  if (purchase.applicationId !== envOptional('GOOGLE_PLAY_PACKAGE_NAME') || !googleEnvironmentsFor(userId).includes(purchase.environment)) throw new Error('Purchase application or environment is not eligible')
 ...
 * License-test rows count only while their owner may hold test purchases.
 */
export async function activeStoreSubscription(userId: string, now = new Date()) {
  ...
  const purchases = await StorePurchase.find({ userId, platform: 'google', applicationId, environment: { $in: googleEnvironmentsFor(userId) }, entitlementState: 'active' }).lean()

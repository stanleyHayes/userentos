...
- New-mover offers are no longer targeted using the user's lease. They are shown to everyone with a "For new movers" tag. Businesses are told nothing when a lease goes live (the per-lease notice and a later weekly per-city count were both removed).
...
| Apple: Developer's Advertising or Marketing purpose | Not ticked for any data type. The apps show no ads and RentOS sends users no marketing messages. |
...
| Google: Advertising ID | No (not used). `AD_ID` is in `app.json` `blockedPermissions`; confirm it is absent from the release AAB's merged manifest (`bundletool dump manifest`) before answering. |
...
- **Backups**: deleted data can remain in database backups until they expire. Account closures, and the documents, listings, listing photos, reviews, business listings, payout accounts and webhooks users delete, are recorded in an erasure ledger and re-applied, with the retention purge, before a restored copy is used ([backup-restore.md](backup-restore.md)). Other deletions (for example unblocking someone or removing a saved property) are not recorded.
